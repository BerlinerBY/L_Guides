...
3. FontForge. Для прагляду і рэдагавання шрыфту. P.S. Апцыянальна.
...
5. Абіраем згенераваны TMP-шрыфт і націскаем у акенцы Inspector на кнопку **"Update Atlas Texture"**, пасля якога адкрыецца акенца рэдагавання і генерацыі шрыфту.
...
6. Абіраем параметры генератара	
    - **Sampling Point Size** -> 90 (памер, калі не ўсе літары/знакі змяшчаюцца на атласе)
	- **Font Padding** -> 5 (аптымальны варыянт)
	- **Packing Method** -> Optimum
	- **Atlas Resolution** -> 1024х1024  (але можна і павялічыць памер, калі не ўсе літары/знакі змяшчаюцца на атласе);
	- **Character Set** - > (Custom Characters)

        Пры генерацыі быў ужыты менавіта такі набор літар/знакаў, бо ён складаецца з літараў/знакаў, што ўжо былі запісаны ў шрыфт + першапачаткова адсутных у ім літар беларускай мовы:
...
7. Націскаем **Generate Font Atlas** і правяраем, каб усе літары/знакі трапілі на выяву
...
11. Дадаём у каранёвы каталог /Assets файл TMP_ExportData.cs і з яго дапамогай дастаём патрэбную нам інфармацыю аб TMP-шрыфце. Пасля выкарыстання, інфармацыя будзе экспартавана ў файл **tmp_font_full_export.json**
...
16. З дапамогай UABEA дастаём з сабранага намі праекта Texture2D з назвай шрыфту: **Патрэбны asset** -> **Plugins** -> **Export Texture**
